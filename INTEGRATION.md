...
## Telegram webhook required for `/audio`

The downloadable patch now includes and mounts a dedicated webhook route:

```text
POST /api/telegram/webhook
```

FastAPI startup automatically calls Telegram `setWebhook` and verifies the
result with `getWebhookInfo`. Configure Render with:

```env
TELEGRAM_WEBHOOK_SECRET=GENERATE_A_URL_SAFE_RANDOM_SECRET
TELEGRAM_WEBHOOK_URL=https://supporter-ipio.onrender.com/api/telegram/webhook
TELEGRAM_AUTO_CONFIGURE_WEBHOOK=true
```

The secret may contain only letters, digits, `_`, and `-`. Telegram sends it in
`X-Telegram-Bot-Api-Secret-Token`, and the route rejects mismatches.

After deployment, check `/health` for:

```text
audioTelegramWebhookRouteConfigured: true
audioTelegramWebhookConfigured: true
audioStorageReady: true
audioTelegramWebhookErrorCode: null
```

You can also run `python scripts/check_telegram_webhook.py` from a trusted
server environment. The script reads the bot token from the environment and
never prints it.


## 9. Backend review improvements in v2.2.0

- Telegram and Supabase downloads are streamed with hard byte limits.
- Missing metadata and recent storage failures are cached briefly to reduce repeated network traffic.
- Concurrent duplicate Telegram updates are serialized.
- Failed manifest publication triggers best-effort cleanup of the newly uploaded version.
- Listed administrators may use private chat when enabled, even when `TELEGRAM_CHAT_ID` is a group.
- Supabase publishable and `anon` keys are rejected before network requests.
- Metadata supports ETag revalidation with `If-None-Match`.

See `BACKEND_REVIEW_V2.2.0.md` for the complete verified change list.
